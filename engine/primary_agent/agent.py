...
from engine.primary_agent.conversation import (
    conversation_manager, ConversationState, CreationStep
)
...
class PrimaryAgent:
    ...
    def __init__(
        ...
        self.glass_box = GlassBoxLogger(project_id=project_id, broadcast_fn=broadcast_fn)
    ...
    async def process(self, user_message: str) -> dict:
        ...
        if self.state.step == CreationStep.DESCRIBING:
            ...
            response_text, next_action = await self._handle_reviewing_rules(user_message)
    ...
    async def _handle_questioning(self, user_message: str) -> Tuple[str, Optional[str]]:
        ...
        await self._broadcast_glass_box("SYSTEM", "Generating constitutional rule files...")
    ...
    async def _handle_reviewing_rules(self, user_message: str) -> Tuple[str, Optional[str]]:
        """User is reviewing rule files — wait for approval or handle changes."""
        
        if "approve" in user_message.lower() or "looks good" in user_message.lower() or "start" in user_message.lower():
    ...
    async def _handle_training_complete(self, project_id: str):
        ...
        await self._broadcast_glass_box("SYSTEM", "Training complete — building frontend...")
        
        # Start DOM server
        import httpx
        async with httpx.AsyncClient() as client:
            try:
                await client.post("http://localhost:8080/dom-server/start", json={
                    "project_id": self.project_id,
                    "port": 5000
                })
            except Exception as e:
        ...
        config = json.loads(project_config_path.read_text()) if project_config_path.exists() else {}
        ...
        await self._broadcast_glass_box("SYSTEM", "Frontend built — app ready for preview")
