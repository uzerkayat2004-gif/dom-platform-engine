...
def validate_path(project_id: str) -> Path:
    ...
    try:
        project_path.resolve().relative_to(Path("projects").resolve())
    except ValueError:
...
async def chat(data: ChatMessage):
    ...
    try:
        ...
        if "invalid_api_key" in err or "AuthenticationError" in err or "401" in err:
            ...
            friendly = (
                "Rate limit hit. Please wait a moment before sending another message."
            )
...
async def run_training(project_id: str, provider: str, api_key: str):
    ...
    await broadcast(
        {
            ...
            "message": "Phase 1: Generating training data...",
    ...
    await broadcast(
        {
            "type": "training_progress",
            "message": "Phase 2: Fine-Tuning the model...",
            "project_id": project_id,
        }
    )
